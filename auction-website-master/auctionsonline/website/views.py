...
def raise_bid(request, auction_id):
    ...
    try:
        if request.session['username']:
            ...
            #userDetails = UserDetails.objects.filter(username=request.session['username'])
            print(userDetails, flush=True)
            if userDetails[0].balance > 0.0:
